...
(function () {
	...
	fullScroll.prototype.buildPublicFunctions = function () {
		...
		this.animateScroll = function () {
			...
			var currentContainer = document.querySelectorAll('#main > section')[this.defaults.currentPosition];

			for(var i = 0; i < document.querySelectorAll('#main > section').length; i++) {
				document.querySelectorAll('#main > section')[i].classList.remove('visible')
				if(i == this.defaults.currentPosition) {
					document.querySelectorAll('#main > section')[i].classList.add('visible')
				}
			}
